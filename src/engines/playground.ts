...
const intake = (t: Timeline) =>
  t
    .node('client', 'info', 'Opens a server-streaming RPC: 96-page document, wants progress as it goes.', 0)
...
const scenarios: Record<ScenarioId, (t: Timeline) => ScenarioEvent[]> = {
  'happy-path': (t) =>
    intake(t)
      ...
      .node('client', 'ok', 'Receives 12/96 … 96/96, then the final result. Stream closes cleanly.', 800)
      .done(),
  ...
  'malformed-output': (t) =>
    intake(t)
      ...
      .edge('proxy-client', 'Progress frame')
      .node('client', 'recover', 'Sees one slower batch and a complete result. No error surfaced.', 800)
      .done(),
  ...
  'endpoint-timeout': (t) =>
    intake(t)
      ...
      .node('gateway', 'warn', 'A placed in cooldown for 20 s; failure class = timeout, not quota.', 500)
      ...
      .node('service', 'recover', 'Batch completes 7 s late. Concurrency limiter never over-subscribed.')
      .edge('service-proxy', 'Progress frame')
      .edge('proxy-client', 'Progress frame')
      .node('ep-a', 'recover', 'Cooldown expires; a single probe succeeds → back in rotation.', 1500)
  ...
  'client-cancel': (t) =>
    intake(t)
      ...
      .node('gateway', 'warn', 'Upstream HTTP requests aborted; tokens already generated are still billed and attributed.')
      .node('ep-a', 'info', 'Generation stops on connection close.')
      .node('service', 'recover', 'Admission slot released within 50 ms. Nothing leaks: no orphan tasks, no half-written output.', 600)
      .node('client', 'recover', 'A new request from the same user gets a fresh slot immediately.', 800)
      .done(),
  ...
  'provider-outage': (t) =>
    intake(t)
      ...
      .node('gateway', 'warn', 'B: failure 1/3. Two regional endpoints down together — looks like a provider incident.', 300)
      .edge('gateway-c', 'Retry on C')
      .node('ep-c', 'ok', 'Returns in 4.4 s.')
      .node('gateway', 'fail', 'A and B cross the failure threshold → circuit OPEN for both. All traffic to C; admission cap lowered so C is not overrun.', 600)
      .edge('gateway-service', 'Result; backpressure signal: reduce batch concurrency')
      .node('service', 'warn', 'Concurrency limiter drops 16 → 6. Throughput down, correctness unchanged.')
      .edge('service-proxy', 'Progress frame')
      .edge('proxy-client', 'Progress frame')
      .node('gateway', 'info', 'After 30 s: circuit HALF-OPEN. One probe request each to A and B.', 1500)
      .node('ep-a', 'ok', 'Probe succeeds.', 400)
      .node('gateway', 'recover', 'A enters probation: gets 10 % of traffic for 60 s. A single failure during probation re-opens the circuit (anti-flap).', 500)
  ...
  'proxy-idle-kill': (t) =>
    intake(t)
      ...
      .node('proxy', 'warn', '30 s without a byte on the response stream. Idle timeout is 60 s.', 2000)
      .node('proxy', 'fail', 'Without heartbeats this is where the proxy would close the stream and the client would see UNAVAILABLE after 2 minutes of real work.', 1500)
      .node('service', 'info', 'Queue heartbeat: an empty progress frame is emitted every 15 s while any task is in flight, independent of model output.', 400)
      ...
      .edge('service-proxy', 'Progress frame')
      .edge('proxy-client', 'Progress frame')
      .node('client', 'recover', 'A 6-minute extraction completes over a proxy configured for 60 s. Cancellation still works at any point.', 800)
      .done(),
};
